...
object Chats : IntIdTable() {
    ...
    suspend fun all() = databaseScope.async {
        return@async transaction { Chats.selectAll().map { it[chatId] } }
    }.await()

    fun subscribeChat(title: String, id: Long) = databaseScope.launch {
        transaction {
            Chats.insert {
                it[name] = title
                it[chatId] = id
    ...
    fun unsubscribeChat(id: Long) = databaseScope.launch {
        transaction {
            Chats.deleteWhere { chatId eq id }
        }
    }

    suspend fun contains(id: Long) = databaseScope.async {
        transaction {
